  ...
  try {
    ...
    if (isQuery === true) {
      ...
      //--------------------------------------------------------------
        ...
        let response = await networkObj.contract.evaluateTransaction(func, args); //Error
...
exports.signUpVoter = async function (voterId, pwd, name, email, role) {
  console.log('VoterId');
  console.log(voterId);
  ...
  if(!voterId || !name || !pwd || !email || !role) {
    let response = {};
  ...
  try {
    ...
    //Check to see if we`vw already enrolled the user.
    const userExists = await wallet.exists(voterId);
    if(userExists){
      let response = {};
      console.error('An identity for the user ${voterId} already exists in the wallet');
