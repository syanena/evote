...
// get Icraft Voter info, create voter object, and upadte state with their voterId
app.post('/signUpVoter', async (req, res) => {
  console.log('req.body: ');
  console.log(req.body);
  let voterId = req.body.voterId;

  //first create the identity for the voter and add to wallet
  let response = await network.signUpVoter(voterId, req.body.pwd, req.body.name, req.body.email, req.body.role);
  console.log('response from signUpVoter: ');
  console.log(response);
  ...
  if(response.error) {
    ...
    let invokeResponse = await network.invoke(networkObj, false, 'signUpVoter', args);
...
//used as a way to login the voter to the app and make sure they haven`t voter before
app.post('/loginVoter', async (req, res) => {
  console.log('req.body: ');
  console.log(req.body);
  let networkObj = await network.connectToNetwork(req.body.voterId);
  console.log('networkObj: ');
  ...
  if(networkObj.error) {
   ...
   req.body = JSON.stringify(req.body);
   let args = [req.body];
  let invokeResponse = await network.invoke(networkObj, true, 'readToAsset', args);
  //let invokeResponse = await network.invoke(networkObj, true, 'readMyAsset', req.body.voterId); //Success Case
  if(invokeResponse.error) {
